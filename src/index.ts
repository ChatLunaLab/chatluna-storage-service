...
- **S3 兼容存储**：支持 AWS S3、MinIO 等 S3 兼容服务，直接使用对象地址访问
- **WebDAV**：支持 WebDAV 协议的存储服务，直接使用文件地址访问
- **Cloudflare R2**：Cloudflare 的对象存储服务，S3 兼容，直接使用对象地址访问

如果存储后端能生成可访问地址，系统会直接返回该地址，不经过 Koishi 服务器中转。`
    ...
    Schema.union([
        Schema.object({
            ...
            s3Endpoint: Schema.string().description(
                'S3 API 端点 URL（如 https://s3.amazonaws.com 或 MinIO 地址）'
            ),
            ...
            s3SecretAccessKey: Schema.string()
                .description('S3 Secret Key（面板里的 Secret Key）')
                .role('secret'),
            s3PathStyle: Schema.boolean()
                .description('使用 path-style 地址（S3 兼容存储默认建议开启）')
                .default(true)
        }).description('S3 配置'),
        ...
        Schema.object({
            ...
            r2SecretAccessKey: Schema.string()
                .description('R2 Secret Key（面板里的 Secret Key）')
                .role('secret')
